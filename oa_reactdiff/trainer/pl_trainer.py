...
class DDPMModule(LightningModule):
    def __init__(
        ...
        # [新增] 初始化列表，用于手动收集 validation_step 的输出
        # [新增] 训练步骤的输出列表
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        '''
        # 1. 获取数据路径
        datadir_path = Path(self.training_config["datadir"])
        
        if not datadir_path.is_file():
             raise FileNotFoundError(
                 f"Configuration Error: The path '{datadir_path}' is not a file. "
                 "Please ensure 'datadir' points directly to your .npz file."
             )
        ...
        if stage == "fit":
            # 设定划分比例，例如 90% 训练，10% 验证
            total_size = len(full_dataset)
            train_size = int(0.9 * total_size)
            val_size = total_size - train_size
            
            print(f"正在将数据集划分为: 训练集 {train_size} + 验证集 {val_size}")

            # 使用固定种子进行随机划分，保证每次运行结果一致
            ...
            # 注意：self.training_config["reflection"] = False 这种操作
            # 在 Subset 上无法直接生效，但对于您的 R->P 任务这通常不影响。
        elif stage == "test":
    ...
    def train_dataloader(self) -> DataLoader:
        # 检查 dataset 是否是 Subset 类型 (如果是 random_split 生成的)
        ...
        return DataLoader(
            ...
            collate_fn=collate_fn, # <--- 使用修正后的 collate_fn
        )
    ...
    def eval_inplaint_batch(
        ...
        # [新增] 打印原始 RMSD 列表，看看是不是全是 1.0
        print(f"\n[DEBUG RMSD] Raw list: {rmsds}")
        print(f"[DEBUG RMSD] Max: {np.max(rmsds)}, Min: {np.min(rmsds)}")
    ...
    def training_step(self, batch, batch_idx):
        ...
        # [关键新增] 收集所有指标的字典 (确保 loss 是标量)
        info["loss"] = loss.item() # 转换为标量，方便收集
